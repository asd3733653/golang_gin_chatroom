...
func AdminQueryValidator(query *admin_model.AdminQuery) (time.Time, time.Time, error) {
	...
	if query.StartTime == "" || query.EndTime == "" {
		return time.Time{}, time.Time{}, errors.New("請提供起始時間和結束時間")
	}

	// 將字符串轉換為時間
	startTime, err := time.Parse("2006-01-02T15:04:05", query.StartTime)
	if err != nil {
		return time.Time{}, time.Time{}, errors.New("無效的起始時間")
	}
	endTime, err := time.Parse("2006-01-02T15:04:05", query.EndTime)
	if err != nil {
		return time.Time{}, time.Time{}, errors.New("無效的結束時間")
	}

	// 檢查結束時間是否早於起始時間
	if endTime.Before(startTime) {
		return time.Time{}, time.Time{}, errors.New("結束時間不能早於起始時間")
	}
